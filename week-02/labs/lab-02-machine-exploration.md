...
**OS and version:** Windows 11 Home / Version 25H2
...
**Cheat sheet — common processes you'll likely see (not exhaustive, just a starting reference):** | Process Name | Usually Seen On | What It Generally Is | | --- | --- | --- | | Antimalware Core Service | Windows | A security process that protects the system from malware, viruses, and other harmful activity. | | Dell Analytics | Windows (Dell systems) | A Dell service that collects system data and monitors performance trends to help optimize the device. | | App Actions | Windows | A background process that helps apps run smoothly by managing certain Windows actions and shortcuts. | | Dell Diagnostics | Windows (Dell systems) | A tool that checks the health of the computer and monitors hardware status to catch issues early. | | Dell TechHub | Windows (Dell systems) | A central hub for Dell‑specific apps and tools that support system management and updates. |
...
**My Screenshot:** https://github.com/martinemonereau/Martine-cyberfoundations-portfolio/blob/main/assets/screenshots/week-02/machine-exploration.png?raw=true
...
```
What I saw in Part A shows the total the computer originally had versus what I currently have available for use. Part B is a visual way to see what is being used right now. The RAM and storage numbers describe the hardware, while the OS version is about the software.
```
...
```
Dell Analytics is still software, even if it isn’t front‑facing like Teams. It depends on the OS to manage its processes, memory, and permissions, and it relies on hardware like the CPU, storage, and network card to actually run its analytics. Without the OS coordinating things or the hardware doing the real work underneath, Dell Analytics wouldn’t be able to function at all.
```
...
```
Watching CPU usage matters for security because a sudden spike from a process you don’t recognize can mean something is running in the background without your knowledge. That could be malware, a hidden script, or some other unauthorized activity. Even when you’re not doing anything, the system might be doing something you didn’t approve. A cyber professional pays attention to this because unusual CPU behavior is often one of the first signs that something isn’t right.
```
...
```
Task Manager made my diagram feel more real because I could actually see everything running on my computer at once. The diagram shows the layers, but watching it live showed all the apps, background processes, and software that change from one machine to another. Seeing those numbers move in real time made it clear how active the system really is and how fast something unusual can show up, which a static diagram could never capture.
```
